...
spOptionDataWDiv <- data.frame(spOptionDataWDiv %>% 
                                ...
                                mutate(
                                  borrowInterestRate = CalculateBorrowInterestRate(
                                    bidPrice, askPrice, cpFlag, annlPayoutReturn,
                                    closePrice, dateLen, strikePrice),
...
# spOptionDataWDiv <- data.frame(spOptionDataWDiv %>% 
#                                  mutate(interestRateF = annlPayoutReturn * (closePrice/))
...
futuresData <- fread("sp500_future_prices.csv")
head(futuresData)

futuresData$Date <- as.Date(futuresData$Date, format = "%b %d, %Y")
futuresData$year <- as.numeric(substr(futuresData$Date, 1, 4))
futuresData$month <- as.numeric(substr(futuresData$Date, 6, 7))
futuresData$day <- as.numeric(substr(futuresData$Date, 9, 10))
futuresData$Price <- as.numeric(gsub(",", "", futuresData$Price))
futuresData <- futuresData %>% select(year:day, futuresClosePrice = Price)

spOptionDataWDivWFuture <- left_join(spOptionDataWDiv, futuresData,
                                     by = c("startYear" = "year",
                                            "startMonth" = "month",
                                            "startDay" = "day"))

spOptionDataWDivWFuture[15000:15005, ]
spOptionDataWDivWFuture[25000:25005, ]
spOptionDataWDivWFuture[35000:35005, ]
tail(futuresData)

spOptionDataWDivWFuture <- data.frame(spOptionDataWDivWFuture %>%
                                 mutate(interestRateF = annlPayoutReturn
                                        * (closePrice/futuresClosePrice)^(-365/dateLen)))
...
spOptionDataWDivWFuture <- data.frame(spOptionDataWDivWFuture %>%
                                        mutate(newAPR = (
                                          (
                                            1 + sqrt(
                                              1 + 4*dividends / (
                                                futuresClosePrice * interestRateF^(
                                                  -dateLen/365
                                                )
                                              )
                                            )
...
spOptionDataWDivWFuture <- data.frame(spOptionDataWDivWFuture %>%
                                        mutate(newClosePrice = futuresClosePrice
                                               * (interestRateF / newAPR)^(-dateLen/365)))

newOldDiff <- spOptionDataWDivWFuture$annlPayoutReturn != spOptionDataWDivWFuture$newAPR
spOptionDataWDivWFuture[head(which(newOldDiff)), ]
spOptionDataWDivWFuture[1160:1175, ]
newOldDiff[1160:1175]
head(which(newOldDiff))
summary(newOldDiff)

write.csv(spOptionDataWDivWFuture, "spOptionDataWDivWFuture", row.names = F)
